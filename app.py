...
if st.session_state.page == 'input':
    ...
    # 학생 정보
    search = st.selectbox("기존 학생 불러오기", ["직접 입력"] + sorted(list(history.keys())))
    ...
    st.divider()
    st.subheader("📖 부교재 선택 (복수 선택 가능)")
    
    # --- 부교재 1 (뿌리깊은) ---
    use_sub1 = st.checkbox("1. 뿌리깊은 초등국어")
    sub1_final = ""
    if use_sub1:
        ...
        default_val = SUB_BOOK_INFO.get(step, {}).get(unit, "어휘 학습을 완료했습니다.")
        sub1_msg = st.text_area("학습 포인트 (해설지 기반)", value=default_val, key=f"sub1_{step}_{unit}")
        sub1_final = f"• 부교재: 뿌리깊은초등국어 [{step}-{unit}] ({s1_time})\n[학습 포인트]\n{sub1_msg}\n\n"

    # --- 부교재 2 (어휘 문해력) ---
    use_sub2 = st.checkbox("2. 어휘가 문해력이다")
    sub2_final = ""
    if use_sub2:
        ...
        # ❗ 여기가 선생님이 찾으시던 입력칸입니다 ❗
        sub2_msg = st.text_area("어휘가 문해력이다 상세 활동 내용", placeholder="배운 어휘나 활동 내용을 자유롭게 적으세요.", key=f"sub2_{lv}_{ut}")
        sub2_final = f"• 부교재: 어휘가 문해력이다 [{lv}-{ut}] ({s2_time})\n  - 활동: {sub2_msg}\n\n"

    # --- 부교재 3 (독서평설) ---
    use_sub3 = st.checkbox("3. 독서평설")
    sub3_final = ""
    if use_sub3:
        s3_type = st.selectbox("구분", ["초등", "중등"])
        sub3_msg = st.text_area("독서평설 학습 내용", placeholder="읽은 주제나 요약 내용을 적으세요.", key="sub3_msg")
        sub3_final = f"• 부교재: 독서평설({s3_type})\n  - 내용: {sub3_msg}\n\n"

    # --- 부교재 4 (신문) ---
    use_sub4 = st.checkbox("4. 신문")
    sub4_final = ""
    if use_sub4:
        sub4_msg = st.text_area("신문 활용 활동 내용", placeholder="신문 기사 요약이나 활동을 적으세요.", key="sub4_msg")
        sub4_final = f"• 부교재: 신문 활동\n  - 내용: {sub4_msg}\n\n"

    st.divider()
    st.subheader("✍️ 선생님 피드백")
    footer = "아이에게 '읽은 책 자랑'을 꼭 시켜주세요~ 더불어 많은 칭찬과 격려부탁드립니다. 감사합니다!"
    comment = st.text_area("종합 의견", value=footer, height=120)
