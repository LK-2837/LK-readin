...
SUB_BOOK_INFO = {
    ...
    **{f"{k}단계": {f"{i}회차": f"{k} {i}회차 어휘 학습을 통해 문장력을 다졌습니다." for i in range(1, 41)} for k in range(1, 7) if f"{k}단계" not in ["1단계", "2단계", "4단계", "5단계"]}
}
...
if st.session_state.page == 'input':
    ...
    with st.expander("🔍 기존 학생 정보 불러오기"):
        search = st.selectbox("학생 선택", ["직접 입력"] + sorted(list(history.keys())))
    ...
    selected_level = st.selectbox("주교재 레벨", ["기초레벨"] + [f"{i}레벨" for i in range(1, 11)], index=0)
    v_book_name = st.text_input("도서명", value=last_rec.get("v_book", ""))
    
    cv1, cv2, cv3, cv4 = st.columns(4)
    v_unit1 = cv1.selectbox("회차", [f"{i}회차" for i in range(1, 5)], key="v1")
    v_corr1 = cv2.number_input("맞은개수", 0, 10, 10, key="c1")
    v_tot1 = cv3.number_input("전체개수", 1, 10, 10, key="t1")
    v_status1 = cv4.selectbox("결과", ["통과", "미통과"], key="s1")

    st.divider()
    # --- [부교재 영역] ---
    st.subheader("📖 3. 부교재 학습 (해당되는 것만 체크)")
    
    # 부교재 1
    use_sub1 = st.checkbox("뿌리깊은 초등국어")
    sub1_data = ""
    if use_sub1:
        s1_c1, s1_c2 = st.columns(2)
        s1_step = s1_c1.selectbox("단계", [f"{i}단계" for i in range(1, 7)])
        s1_unit = s1_c2.selectbox("회차", [f"{i}회차" for i in range(1, 41)])
        s1_m = st.selectbox("소요시간(분)", [f"{i}분" for i in range(61)], index=0)
        default_val = SUB_BOOK_INFO.get(s1_step, {}).get(s1_unit, "어휘 학습을 완료했습니다.")
        sub1_data = st.text_area("학습 포인트 수정", value=default_val, key=f"area_{s1_step}_{s1_unit}")
        sub1_final = f"• 부교재: 뿌리깊은초등국어 [{s1_step}-{s1_unit}] ({s1_m})\n[학습 포인트]\n{sub1_data}\n\n"

    # 부교재 2 (어휘가 문해력이다)
    use_sub2 = st.checkbox("어휘가 문해력이다")
    sub2_final = ""
    if use_sub2:
        s2_c1, s2_c2 = st.columns(2)
        s2_lv = s2_c1.selectbox("레벨 ", VOCAB_LITERACY_LEVELS)
        s2_ut = s2_c2.selectbox("회차 ", [f"{i}회차" for i in range(1, 41)])
        s2_m = st.selectbox("소요시간(분) ", [f"{i}분" for i in range(61)], index=0)
        # ⚠️ 여기가 포인트입니다. 선생님이 자유롭게 적으시는 칸!
        sub2_custom = st.text_area("활동 상세 내용 입력", placeholder="어떤 어휘를 배웠는지, 활동 내용은 무엇인지 적어주세요.", key=f"custom_{s2_lv}_{s2_ut}")
        sub2_final = f"• 부교재: 어휘가 문해력이다 [{s2_lv}-{s2_ut}] ({s2_m})\n  - 활동: {sub2_custom}\n\n"

    # 부교재 3 (독서평설)
    use_sub3 = st.checkbox("독서평설")
    sub3_final = ""
    if use_sub3:
        s3_type = st.selectbox("구분", ["초등", "중등"])
        sub3_custom = st.text_area("독서평설 상세 내용", placeholder="주요 기사 제목이나 요약 내용을 적어주세요.")
        sub3_final = f"• 부교재: 독서평설({s3_type})\n  - 내용: {sub3_custom}\n\n"

    # 부교재 4 (신문)
    use_sub4 = st.checkbox("신문")
    sub4_final = ""
    if use_sub4:
        sub4_custom = st.text_area("신문 상세 내용", placeholder="신문 활용 교육(NIE) 내용을 적어주세요.")
    ...
    if st.button("🚀 리포트 생성", type="primary"):
        if not name: st.warning("이름을 입력하세요.")
            ...
            # 주교재 리포트 빌드
            main_report = f"■ 도서명: {v_book_name}\n - 진단결과: [{v_unit1}] [{v_status1}] {v_corr1}/{v_tot1}"
            
            # 부교재 리포트 합치기
            sub_total = ""
            if use_sub1: sub_total += sub1_final
            if use_sub2: sub_total += sub2_final
            if use_sub3: sub_total += sub3_final
            if use_sub4: sub_total += sub4_final
...
{teacher_comment}"""
    ...
    st.title("📄 생성된 리포트")
    st.text_area("전체 내용 복사 (Ctrl+A -> Ctrl+C)", st.session_state.final_text, height=600)
